...
parse = function(input, options, callback) {
	...
	if (!Array.isArray(input)) {
      ...
      if (fileName.match(/(\.md)$/)) {
        markdownFiles.push(input.length - 1);
      }
		}
		data.files.sort();
	}
	...
	data.section_refs = [];
	for (i = 0; i < l; i += 1) {
    if (markdownFiles.indexOf(i) == -1) {
		  data = parseChunk(data, input[i], options) || data;
    }
    else {
        console.log('MARKDOWN!');
        data = parseChunk(data, input[i], options, true) || data;
    }
	}
 ...
 * @return {Object}
 */
parseChunk = function(data, input, options, markdown) {
	...
	blocks = findBlocks(input, options, markdown);
	l = blocks.length;
 ...
 */
findBlocks = function(input, options, markdown) {
